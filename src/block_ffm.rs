...

 {
    ...
    fn new_forward_only_without_weights(&self) -> Result<Box<dyn BlockTrait>, Box<dyn Error>> {
     ...
     fn allocate_and_init_weights(&mut self, mi: &model_instance::ModelInstance) {
		 // Partially based on https://link.springer.com/article/10.1007/s10462-021-10033-z
         ...
         if mi.ffm_k > 0 {
			 if mi.ffm_initialization_type == String::from("default") {
			 ...
			 } else if mi.ffm_initialization_type == String::from("xavier_norm_init") {
				 // weight = U [-(sqrt(6)/sqrt(n + m)), sqrt(6)/sqrt(n + m)]
				 println!("test Xavier");
			 } else if mi.ffm_initialization_type == String::from("khe_init") {
				 // G (0.0, sqrt(2/n))
			 } else if mi.ffm_initialization_type == String::from("random_init") {
				 // random
			 } else if mi.ffm_initialization_type == String::from("random_relu") {
				 // random + activation
			 }
		 }
     }
